...
class Config:
    ...
    ## CHANGELOG: Increased worker simulations for higher-quality training data.
    mcts_sims_worker: int = 10_000         # Sims per move in self-play.
    ## CHANGELOG: Added MCTS batch size to config for better GPU utilization during search.
    ...
    ## CHANGELOG: Made network depth configurable. Increased for a more powerful model.
    ...
    ## CHANGELOG: Increased training batch size for faster training steps on a powerful GPU.
    ...
    ## CHANGELOG: Increased number of parallel workers to better saturate the GPU.
...
class PolicyValueNet(nn.Module):
    ## CHANGELOG: Network size is now driven by the Config class.
...
## CHANGELOG: MCTS now uses the configurable batch size from CFG.
...
def self_play_worker(args):
    ...
    # Each worker has its own model instance on the GPU.
    # This is VRAM-intensive but minimizes latency vs. a central server.
