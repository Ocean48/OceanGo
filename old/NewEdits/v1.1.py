...
def expand(node):
    while node.unvisited_moves:
        ...
        print(f"Expanding with move {move}, initializing visit count.")
        return child_node
    return node
...
def best_child(node):
    ...
    chosen = max(choices_weights, key=lambda x: x[1])[0]
    print(f"Best child selected: Move {chosen.move}, Visit Count: {chosen.visit_count}")
    return chosen
...
def backpropagate(node, result):
    while node is not None:
        ...
        print(f"Backpropagating move {node.move}, Updated Visit Count: {node.visit_count}, Total Value: {node.total_value}")
        node = node.parent
...
def ai_play(game, iterations=ITERATIONS, num_processes=PROCESSES_NUM):
    ...
    print(f"AI's best move: {best_move} with visit count: {move_visits_total[best_move]}")
    print("Detailed visit counts for all moves considered:")
    for move, count in move_visits_total.items():
        print(f"Move: {move}, Visit Count: {count}")
    return best_move
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if game.current_player == 1:
                ...
                print("AI is about to make the move:")
                game.render()
