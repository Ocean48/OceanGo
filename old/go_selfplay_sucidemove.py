...
class GoGame:
    ...
    def is_valid_move(self, x, y):
        ...
        if 0 <= x < self.board_size and 0 <= y < self.board_size and self.board[x][y] == 0:
            ...
            if (x, y) != self.ko:
                # Check for the "no suicide move" rule
                temp_board = copy.deepcopy(self.board)
                temp_board[x][y] = self.current_player

                # Check if the move would capture any adjacent opponent groups
                    ...
                    if 0 <= i < self.board_size and 0 <= j < self.board_size:
                        if temp_board[i][j] == 3 - self.current_player:
                            ...
                            if not self.has_liberties(group):
                                return False  # The move would result in an immediate capture
...
def ai_play(board, iterations=1000):
    ...
    # Filter out moves that would result in suicide
    legal_moves = root.unvisited_moves
    filtered_moves = []

    for move in legal_moves:
        x, y = move
        temp_board = copy.deepcopy(root.state)
        temp_board.make_move(x, y)

        # Check if the move would capture any adjacent opponent groups
        is_valid = True
        for i, j in [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]:
            if 0 <= i < root.state.board_size and 0 <= j < root.state.board_size:
                if temp_board.board[i][j] == 3 - root.state.current_player:
                    group = temp_board.find_group(i, j)
                    if not temp_board.has_liberties(group):
                        is_valid = False
                        break
    ...
    root.unvisited_moves = filtered_moves
...
def self_play(iterations=1000):
    ...
    while not game.is_game_over():
        ...
        if game.current_player == 1:
            ai_move = ai_play(game, iterations)  # Let AI make a move
            game.make_move(*ai_move)
        else:
            ai_move = ai_play(game, iterations)  # AI vs. AI, let AI make the move
            game.make_move(*ai_move)
