...
class GoGame:
    ...
    def copy(self):
        g = GoGame(self.bs)
        ## CHANGELOG: CRITICAL BUG FIX - Was copying a new empty board instead of self.board.
...
def _cleanup_dead_stones(board, black_captures, white_captures):
    ...
    This is a simple but essential part of the life-and-death resolution.
    """
...
def _resolve_life_and_death(game: GoGame):
    """
    After players pass, play a set number of random moves to resolve
    ambiguous life/death situations before final scoring.
    """
    temp_game = game.copy()
    ...
    for _ in range(CFG.cleanup_moves):
        ...
        if not legal_moves:
            # If no moves left, switch player and try again
            temp_game.switch()
            legal_moves = temp_game.get_legal_moves(include_pass=False)
            if not legal_moves:
                break # Board is full
    ...
    # After the random playout, run the simple cleanup to remove any resulting dead stones.
    final_board, final_black_captures, final_white_captures = _cleanup_dead_stones(
        temp_game.board, temp_game.captured_by_black, temp_game.captured_by_white
    )
    
    # Create a final game state for scoring
    final_game = GoGame()
    final_game.board = final_board
    final_game.captured_by_black = final_black_captures
    final_game.captured_by_white = final_white_captures
    
    return final_game

## CHANGELOG: New helper to render a board state to a string for logging.
...
def mcts(game: GoGame, net: PolicyValueNet, sims: int, temp: float = 1.0, add_noise: bool = False):
    ...
    for _ in range(sims):
        leaf = select(root)
        if leaf.g.game_over():
            scoring_game = _resolve_life_and_death(leaf.g)
...
def self_play_worker(args):
    ...
    if hit_max_moves:
        ...
        final_game_state = _resolve_life_and_death(game)
    ...
    ## CHANGELOG: Log the final board state for the first few workers for debugging.
    if rank < 3:
