...
# Define the MCTS worker function
def mcts_worker(root, iterations, results):
...
def mcts(root, iterations, num_processes):
    # Split the iterations evenly among processes
    iterations_per_process = iterations // num_processes

    # Create a list to store the results of each process
    results = []

    # Create and start multiple processes
    processes = []
    for _ in range(num_processes):
        process = multiprocessing.Process(target=mcts_worker, args=(root, iterations_per_process, results))
...
def ai_play(board, iterations=ITERATIONS, num_processes=PROCESS_NUM):
    root = MCTSTreeNode()
    root.state = board
    root.unvisited_moves = board.get_legal_moves()
    total_results = mcts(root, iterations, num_processes)
    
    if root.children:
        best_move = max(root.children, key=lambda c: c.visit_count)
        return best_move.move
    else:
        # Handle the case where there are no child nodes (e.g., no legal moves)
        return None
...
def main():
    ...
    while not game.is_game_over():
        ...
        if game.current_player == 1:
            ...
            ai_move = ai_play(game, num_processes=num_processes)
            if (ai_move == None):
                break
