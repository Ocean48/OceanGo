...
class Config:
    ...
    ## CHANGELOG: Added key MCTS hyperparameters for easier tuning.
    mcts_c_puct: float = 1.4              # Exploration factor in PUCT.
    dirichlet_alpha: float = 0.03         # Noise for root node exploration.
    start_temp: float = 1.0               # Initial temperature for move selection.
    temp_decay_moves: int = 30            # Moves before temp -> 0 for exploitation.
    ## CHANGELOG: Add a minimum move count before passing is allowed during self-play.
    # This is crucial to prevent the AI from learning to pass prematurely to win via komi.
...
## CHANGELOG: Set the dynamic default for min_moves_for_pass.
...
def log_session_header():
    ...
    ## CHANGELOG: Add blank lines to separate session headers in the log file.
    LOGGER.info("\n\n" + "-" * 80)
...
class GoGame:
    ...
    def make_move(self, x, y):
        ...
        self.ko_point = None
        if num_captured == 1:
            # A ko is only possible if a single stone is captured.
            # The ko point is the now-empty intersection where that stone was.
            diff = prev_board - self.board
            if np.sum(diff != 0) == 2: # One stone placed, one removed
                removed_mask = (prev_board != 0) & (self.board == 0)
                if np.sum(removed_mask) == 1:
                    ko_x, ko_y = np.argwhere(removed_mask)[0]
                    
                    # Final check: is playing at ko_point suicidal? If not, it's a ko.
    ...
    def get_legal_moves(self):
        ...
        ## CHANGELOG: CRITICAL FIX - Prevent premature passing exploit.
        # Only allow passing after a minimum number of moves have been played,
        # or if it is the only legal move available (e.g., board is full).
        if len(self.history) >= CFG.min_moves_for_pass or not moves:
            moves.append(PASS)
    ...
    ## CHANGELOG: Refactored `winner` to `get_score` to return the raw score margin.
    # This allows for more nuanced game outcome analysis, especially for the self-play worker.
    def get_score(self, komi: float = CFG.komi):
        """Returns the final score from Black's perspective (B - (W + Komi))."""
        return _chinese_area(self.board, komi)
...
def select(node: 'Node'):
    while True:
        ...
        ## CHANGELOG: Use configurable C_puct
...
def mcts(game: GoGame, net: PolicyValueNet,
         ...
         batch_size: int = 128, add_noise: bool = False):
    root = Node(game.copy())
    nn_eval([root], net)

    if add_noise and root.ch:
        ## CHANGELOG: Use configurable dirichlet_alpha
    ...
    for _ in range(sims):
        leaf = select(root)
        if leaf.g.game_over():
            ## CHANGELOG: Use the new get_score() method to determine the winner.
            score = leaf.g.get_score()
    ...
    if temp < 1e-3:
        ...
        ## CHANGELOG: Fixed numerical instability causing overflow/NaN errors.
        # The original `v ** (1 / temp)` could overflow with large visit counts
        # and small temperatures. This new approach using logarithms is stable.
        log_visits = np.log(np.maximum(v, 1e-9))  # Epsilon to prevent log(0)
        log_weighted = log_visits / temp
        log_weighted -= np.max(log_weighted) # Stabilize before exponentiation
        w = np.exp(log_weighted)
        
        p = w / w.sum()
        # Fallback in the rare case of numerical issues leading to an invalid distribution
...
def self_play_worker(args):
    ...
    while not game.game_over() and move_ctr < CFG.max_moves:
        ...
        ## CHANGELOG: Implement temperature decay for exploration vs exploitation.
        temp = CFG.start_temp if move_ctr < CFG.temp_decay_moves else 0.01
        pi = mcts(game, net, sims_per_move, temp, add_noise=True)
        if not pi: break
        # network to learn meaningful policies rather than exploiting scoring artifacts.
    ...
    if hit_max_moves:
        ...
        if score_margin < 1e-3:
            ...
            w = 1 if final_score > 0 else 2 # Black or White win

        ## CHANGELOG: CRITICAL FIX - If the game did not hit the move limit but ended with a tiny
        # margin (likely just komi on a sparse board), it's an exploit.
        # Force a draw to prevent the AI from learning this pathological strategy.
        if w != 0 and score_margin <= CFG.komi:
            LOGGER.warning(
                f"Worker {rank} ended on turn {move_ctr} with ambiguous score margin "
                f"({score_margin:.1f} <= komi). Forcing a DRAW."
            )
            w = 0 # Override to draw
    ...
    ## CHANGELOG: Create the final training data. For each state in the game,
    # append the final game outcome from that state's perspective.
    # Value is +1 if the current player eventually won, -1 if they lost.
    training_data = []
    for board_state, player, policy_vec in records:
        outcome = 0
        if w != 0:
            outcome = 1 if player == w else -1
...
def train(net: PolicyValueNet, batch_data, it_idx):
    ...
    ## CHANGELOG: This is the critical fix. The training loop now uses the
    # MCTS-generated policy and the final game outcome as targets, which is
    # the correct AlphaGo Zero methodology.
    boards, players, target_policies, target_values = zip(*batch_data)
    
    X = torch.stack([planes(b, p) for b, p in zip(boards, players)]).to(device_gpu, non_blocking=True)
    ...
    for ep in range(1, CFG.train_epochs + 1):
        ...
        for s in range(0, len(batch_data), CFG.train_batch):
            ...
            ## CHANGELOG: Added policy and value loss to the progress bar for better diagnostics.
            bar.set_postfix(
                loss=f"{avg_loss:.4f}",
                p_loss=f"{avg_p_loss:.4f}",
                v_loss=f"{avg_v_loss:.4f}",
                vram=f"{vram:.0f}MB"
            )
            bar.update()
...
def run_gui(net: PolicyValueNet):
    ...
    ## CHANGELOG: Added font for rendering game info text.
    ...
    def draw(g: GoGame, win_rate: float):
        ...
        ## CHANGELOG: Render all game information on the screen.
    ...
    while True:
        if game.game_over():
            ...
            ## CHANGELOG: Use get_score to report final game result.
            final_score = game.get_score()
        ...
        ## CHANGELOG: Evaluate position once per turn to get win rate.
        root_node = Node(game.copy())
        nn_eval([root_node], net)
        # Value is from the current player's perspective, between -1 and 1.
        # Convert to a 0-100% win probability.
...
def main():
    ...
    ## CHANGELOG: Shortened the checkpoint filename for better readability.
    ...
    for it in outer:
        ...
        ## CHANGELOG: Added stats tracking for each self-play iteration.
        ...
        ## CHANGELOG: Log the detailed stats collected from the self-play games.
